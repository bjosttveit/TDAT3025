...
class DQN:
    def __init__(self, model, env, gamma=0.95, optimizer=keras.optimizers.Adam(lr=0.001), lossfunc=keras.losses.MSE, exp_iterations=3, mem_size=128*100, batch_size=128, expl_max=1.0, expl_min=0.01, expl_decay=0.95):
        self.model = model
        self.env = env
    ...
    def action(self, q_values):
        if np.random.rand() < self.expl_rate:
            ...
            return random.randrange(1,m) #tilfeldig er aldri å gjøre ingenting
        ...
        try:
            while True:
                ...
                for i in range(len(episode_memory[0])):
                    self.weights.append(np.power(episode_memory[3][i]-episode_memory[1][i][episode_memory[2][i]], 2))
...
dqn = DQN(model, env, expl_max=0.0, expl_min=0.0)
