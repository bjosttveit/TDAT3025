...
class TokyoDrifter(keras.Model):
    def __init__(self, frames=5, **kwargs):
        ...
        self.conv1 = layers.Conv2D(128, (6, 6), activation='relu')
        self.pool1 = layers.MaxPool2D(2, 2)
        self.conv1 = layers.Conv2D(256, (6, 6), activation='relu')
        self.pool1 = layers.MaxPool2D(2, 2)
...
class DQN:
    def __init__(self, model, env, gamma=0.95, optimizer=keras.optimizers.Adam(lr=0.001), lossfunc=keras.losses.MSE, frames=5, exp_iterations=5, mem_size=128*1000, batch_size=128, expl_max=1.0, expl_min=0.01, expl_decay=0.95):
        self.model = model
        self.env = env
    ...
    def plotReward(self):
        avg_rewards = pd.DataFrame(self.rewards).rolling(10, min_periods=1).mean()
...
if test:
    model.load_weights("model 12-11-2019 15-18-24 1.72.h5")

dqn = DQN(model, env, expl_max=1.0, expl_min=0.01, expl_decay=0.995, gamma=0.95, exp_iterations=1, frames=5)
