...
class DQN:
    def __init__(self, model, env, gamma=0.95, optimizer=keras.optimizers.Adam(lr=0.01), lossfunc=keras.losses.MSE, exp_iterations=5, mem_size=128*25, batch_size=128, expl_max=1.0, expl_min=0.01, expl_decay=0.95):
        self.model = model
        self.env = env
    ...
    def action(self, q_values):
        if self.actint > 0:
            self.actint += 1
            if self.actint > 3: self.actint = 0
            return np.argmax(q_values)
        
        if self.rndint > 0:
            self.rndint += 1
            if self.rndint > 3: self.rndint = 0
            return self.rndact
        
        if np.random.rand() < self.expl_rate:
            self.rndint = 1
            m = 9 #I starten vil tilfeldig valg være begrenset
            if self.expl_rate > 0.9: m = 2
            elif self.expl_rate > 0.5: m = 4
            elif self.expl_rate > 0.3: m = 6
            elif self.expl_rate > 0.1: m = 7
        ...
        try:
            while True:
                ...
                for i in range(len(episode_memory[0])):
                    self.weights.append(np.sqrt(np.power(episode_memory[3][i]-episode_memory[1][i][episode_memory[2][i]], 2)))
...
dqn = DQN(model, env, expl_max=1, expl_min=0.01, expl_decay=0.975, gamma=0.99)
